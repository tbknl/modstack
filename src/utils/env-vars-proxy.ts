...
export const makeEnvProxy = (envVars: EnvVars, options?: { onLockedAccess?: ({ key }: { key: string }) => (boolean | void) }) => {
	let locked = false;

	const checkLock = (key: string) => {
		if (locked) {
			if (!options?.onLockedAccess || !options?.onLockedAccess({ key })) {
				throw new Error(`Unallowed access to env var '${key}' outside configuration phase.`);
			}
		}
	};
	...
	const envVarsProxy = new Proxy(envVars, {
		...
		set: (_target, prop) => {
			throw new Error(`Cannot set property ${prop.toString()} on read-only env vars proxy.`);
		},
	});
