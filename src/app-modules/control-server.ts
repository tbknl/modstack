	...
	initialize: async (
					...
					Object.entries({
						GET: (url: URL) => {
							...
							return {
								...
								content: JSON.stringify(statusData ?? null),
							};
						},
					}),
				),
					...
					Object.entries({
						GET: (url: URL) => {
							...
							return {
								status: 200,
								headers: { "Content-Type": "application/json" },
								content: JSON.stringify(infoData ?? null),
							};
						},
					}),
				),
						...
						POST: () => {
							if (allowStop) {
								if (lifecycle.status().inStoppablePhase) {
									logger?.info(
										"Stopping after control server received stop request.",
									);
									lifecycle.stop();
									return {
										status: 200,
									...
									return {
										status: 409,
										headers: { "Content-Type": "text/plain" },
										content: "Not in a stoppable phase",
									};
								}
							} else {
								return {
		...
		await new Promise<void>((resolve, reject) => {
			...
			server.listen(config.port, config.host, () => {
				logger?.info(
					`Control server listening for http requests on ${server.address()?.toString()}.`,
				);
				resolve();
			});
		});
