...
const makeModState = <Cfg, Inst, Deps extends { readonly [Name: string]: unknown }>(
  ...
  depMap: { [K in keyof Deps]: { getInstance(): Deps[K]; finalize(): Promise<boolean> } },
): ModState<Inst> => {
			...
			if (!finalizationPromise) {
				finalizationPromise = (async () => {
					...
					if (Object.keys(depMap).length) {
						logger.info(`[${name}] Waiting for dependencies to finalize first.`);
						await Promise.all(Object.values(depMap).map((dep) => dep.finalize()));
					}
...
const makeLifecycle = (logger: Logger, modStates: readonly ModState<unknown>[]) => {
  ...
  return {
    ...
    async stop() { // TODO: Only trigger stop (synchronous)!
    ...
    async stopped() {
      // TODO: Wait until application is finalized.
    },
    status() {
      return {
        phase,
...
const makeModstack = <A extends { readonly [Name: string]: ModState<unknown> }>(
  ...
  ({
    add: (name, mod, depKeys) => {
      ...
      ) as unknown as { [DK in keyof ModParams<typeof mod>['D']]: { getInstance: () => ModParams<typeof mod>['D'][DK]; finalize: () => Promise<boolean> } }; // TODO: Type!
